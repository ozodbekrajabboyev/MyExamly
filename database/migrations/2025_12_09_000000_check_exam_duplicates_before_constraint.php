...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $duplicates = DB::table('exams')
            ->select('sinf_id', 'subject_id', 'type', 'serial_number', DB::raw('count(*) as duplicate_count'))
        ...
        if ($duplicates->count() > 0) {
            ...
            foreach ($duplicates as $duplicate) {
                echo "Sinf ID: {$duplicate->sinf_id}, Subject ID: {$duplicate->subject_id}, Type: {$duplicate->type}, Serial: {$duplicate->serial_number} (Count: {$duplicate->duplicate_count})\n";
            }
